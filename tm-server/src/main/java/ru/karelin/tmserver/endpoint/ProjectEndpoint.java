...
public class ProjectEndpoint {
    ...
    public ProjectEndpoint(@NotNull final ProjectService projectService, @NotNull SessionService sessionServiceImpl) {
        this.projectService = projectService;
        this.sessionServiceImpl = sessionServiceImpl;
    }

    @WebMethod
    public List<Project> getProjectList(@WebParam(name = "session") final SessionDto session) throws WrongSessionException {
        if (sessionServiceImpl.isSessionExists(session)) return projectService.getList(session.getUserId());
    ...
    public void removeProject(@WebParam(name = "session") final SessionDto session,
        ...
        if (sessionServiceImpl.isSessionExists(session)) projectService.remove(session.getUserId(), id);
    }

    @WebMethod
    public boolean checkProjectId(@WebParam(name = "session") final SessionDto session,
                                  @WebParam(name = "projectId") final String id) throws WrongSessionException {
        if (sessionServiceImpl.isSessionExists(session)) return projectService.checkId(session.getUserId(), id);
        return false;
    }

    @WebMethod
    public Project getProject(@WebParam(name = "session") final SessionDto session,
                              @WebParam(name = "projectId") final String id) throws WrongSessionException {
        if (sessionServiceImpl.isSessionExists(session)) return projectService.getOne(session.getUserId(), id);
        return null;
    }
    ...
    @WebMethod
    public List<Project> getProjectSortedList(@WebParam(name = "session") final SessionDto session,
    ...
    @WebMethod
    public List<Project> getProjectListByKeyword(@WebParam(name = "session") final SessionDto session,
